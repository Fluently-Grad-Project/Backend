...
def get_similar_users(
    ...
    engine = sqlalchemy.create_engine(
        "postgresql://postgres:asdqwe123@localhost/fluently"
    )
    ...
    try:
        ...
        interests = users_df["interests"].apply(
        ...
        practice_encoded = pd.get_dummies(
            users_df["practice_frequency"].fillna("missing"), prefix="practice"
        )

        proficiency_encoded = pd.get_dummies(
            users_df["proficiency_level"].fillna("missing"), prefix="proficiency"
        )
...
def get_similar_users_details(
    user_id: int, db: Session, n_recommendations: int = 5
) -> List[MatchedUserResponse]:
